...
class MSKBProvider(Retriever):
    """
    MSKBProvider is a provider that uses mskb to retrieve documents.
    """
    ...
    def __init__(self):
        api_url = os.getenv("MSKB_API_URL")
        if not api_url:
            raise ValueError("MSKB_API_URL is not set")
        self.api_url = api_url

        api_key = os.getenv("MSKB_API_KEY")
        if not api_key:
            raise ValueError("MSKB_API_KEY is not set")
