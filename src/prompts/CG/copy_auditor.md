...
### 维度 1 补充：本体边触发检查（P3 新增）

在逐句扫描之后，对照下方《本体边触发检查清单》做封闭式对账——当文案涉及某条边对应的业务概念时，该检查项必须执行。这将"逐句找问题"（开放式）升级为"逐边核对"（封闭式），防止小模型遗漏关键维度。

{{ cg_audit_edge_checklist }}

对于每个触发的边检查项，如果文案未满足检查要求，在 `atomic_fact_claims` 中追加一条 `is_faithful: false` 的条目，`deviation_note` 注明缺失的边和具体缺失内容。
