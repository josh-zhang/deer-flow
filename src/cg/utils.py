...
def _fuzzy_match_chunk_map(
    ...
    """
    # 精确匹配
    if doc_title in chunk_maps:
        return chunk_maps[doc_title]

    # 归一化匹配
    norm_query = _normalize_title(doc_title)
    if not norm_query:
        return None

    for key, chunk_map in chunk_maps.items():
        if _normalize_title(key) == norm_query:
            return chunk_map
    ...
    for key, chunk_map in chunk_maps.items():
        ...
        if norm_query in norm_key or norm_key in norm_query:
            logger.info("文档名包含匹配：查询「%s」↔ chunk_map「%s」", doc_title, key)
            return chunk_map
